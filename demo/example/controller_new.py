...
class ControlLoop():
    ...
    def load_model(self, path_exp_yaml, path_model_ckpt, reload = True):
        ...
        # define the model
        self.model = DeepVariationalBayesFilter(use_placeholder=True,
    ...
    def rollout_and_send(self, z0_mean, z0_var):
        ...
        """
        z_paths_mean, u_paths_mean, x_paths_mean, \
        z_paths_var, u_paths_var, x_paths_var = \
            self.roller.evaluate_model_and_obs_policy_from_latent(z0_mean, z0_var,
                                                                  self.t_rollout, self.n_samples_rollout)
        ...
        msg_state = pab.fb_msg(pab.MsgType.NumpyMsg, np.concatenate([x_paths_mean[:, self.state_int], x_paths_var[:, self.state_int]], axis=1))
    ...
    def run(self,
        ...
        self.dc_burnin       = 1
        self.counter_burnin  = 0
        self.wtf_init        = np.zeros((self.model.n_initialobs, 2 * FrankaConfig.robot.n_joints\
                                                                    + FrankaConfig.data.lidar_num\
                                                                    + FrankaConfig.data.img_size))
        ...
        while True:
            ...
            if REGULAR:
               ...
               # self.lidar_fnumber   = msg_lidar.get_fnumber()
                ...
                self.endeff_x        = np.zeros((1, FrankaConfig.robot.n_endeff))
                self.endeff_xd       = np.zeros((1, FrankaConfig.robot.n_endeff))
                self.state_q         = msg_state_data[:, :FrankaConfig.robot.n_joints].reshape((-1, FrankaConfig.robot.n_joints))
                self.state_qd        = msg_state_data[:, FrankaConfig.robot.n_joints:].reshape((-1, FrankaConfig.robot.n_joints))
            ...
            if INIT_ON:
                ...
                if True:
                    ...
                    z_pred_mean, z_pred_var, u_pred_mean, u_pred_var,\
                            x_pred_mean, x_pred_var = self.predictor.step(tmp[(0), :2*self.n_joints].reshape((1, -1)))

                    if time.time() - t_last_rollout > dt_rollout:
                        t_last_rollout = time.time()
                        thread_roll    = threading.Thread(target=self.rollout_and_send,
                                                          args=(z_pred_mean, z_pred_var))
                        thread_roll.start()
...
if __name__ == "__main__":
    ...
    control_loop.run(dt_rollout = dt_rollout,
                        n_samples_rollout = n_samples_rollout,
                        t_rollout  = t_rollout)
