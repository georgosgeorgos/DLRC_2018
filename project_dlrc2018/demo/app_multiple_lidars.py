        ...
        [
            html.Div(
                [
                    ...
                    dcc.Interval(
                        id="interval-component", interval=N_INTERVAL_UPDATE * 1000, n_intervals=N_MAX_INTERVALS
                    ),
                ],
                className="six columns",
            ),
            html.Div([dcc.Graph(id="live-update-bar-chart-clustering-two-classes")], className="three columns"),
            # html.Div([
...
@app.callback(Output("store-data", "children"), [Input("interval-component", "n_intervals")])
def update_data(n):
    data = p.get_data(n)
...
@app.callback(
    Output("live-update-graph-lidars", "figure"),
    [Input("interval-component", "n_intervals"), Input("store-data", "children")],
)
...
@app.callback(
    Output("live-update-graph-probs", "figure"),
    [Input("interval-component", "n_intervals"), Input("store-data", "children")],
)
...
@app.callback(
    Output("live-update-bar-chart-clustering-two-classes", "figure"),
    [Input("interval-component", "n_intervals"), Input("store-data", "children")],
)
def update_barchart_clustering_two(n, json_data):
    ...
    trace = go.Bar(
        ...
        x=probs_classes,
        orientation="h",
        marker=dict(color=["#D6DBDF", "#273746"]),
        width=1,
    )

    layout = Layout(
        xaxis=dict(title="Class probability", range=[0, 1]),
        height=400,
...
if __name__ == "__main__":
    ...
    for lidar_idx in N_LIDAR_IDX:
        cb_lidar = create_callback(lidar_idx)
    app.callback(Output(lidar_idx, "..."), [Input(...)])(cb_lidar)
