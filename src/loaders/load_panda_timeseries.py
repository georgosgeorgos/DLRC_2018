...
class PandaDataSetTimeSeries(data.Dataset):
    ...
    def generate_collision_labels_(self, y, p=0.3, t=0.040):
        ...
        if np.random.random() < p:
            index=np.random.choice(cols, 
                                   np.random.randint(1, y.shape[1]//2), 
                                   replace=False)
            y[:, index] = np.random.random((y.shape[0], len(index))) * 2 #(1 + 0.1 * np.random.randn(index.shape[0])) * t
        else:
            index=[]
    ...
    def generate_collision_labels(self, p=0.3, t=0.040):
        ...
        for row in range(0, y.shape[0], self.n_samples):
            if random() < p:
                ...
                y[(row):((row+s)), index] = np.random.random((s, len(index))) * 2 #(1 + 0.1 * np.random.randn(index.shape[0])) * t
