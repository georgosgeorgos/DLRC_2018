...
app.layout = html.Div(
    children=[
        ...
        dcc.Interval(id="interval", interval=N_UNIT_TIMESTEP * 1000, n_intervals=0, max_intervals=len(df) - 1),
    ],
    className="row",
)
...
def update_subplots(n, row_json):
    ...
    trace1 = go.Scatter(
        x=timesteps, y=np.array(d_train_input[INPUT_IDX])[timesteps], name="train input lidar 3", mode="markers"
    )
    fig.append_trace(trace1, 1, 1)
    # fig.append_trace(trace2, 1, 1)

    fig["layout"].update(title="anomVAE stuff", xaxis={"domain": [0, 1.0]})
